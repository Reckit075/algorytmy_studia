    ...
    public class StringMergeSort
    {
        ...
        private static string[] StringSortArray(string[] arr, int left, int right)
        {
            ...
            if (left + 1 == right)
            {
                ...
                //return new[] {Math.Min(arr[left], arr[right]), Math.Max(arr[left], arr[right])};
                //return new[] { string.Compare("<", "0", StringComparison.Ordinal), string.Compare(">", "0", StringComparison.Ordinal) }
        ...
        private static string[] StringMerge(string[] arr1, string[] arr2)
        {
            ...
            for (int i = 0, j1 = 0, j2 = 0; i < result.Length; i++)
            {
                ...
                {
                    Console.WriteLine(arr1[j1]);
                    Console.WriteLine(arr2[j2]);

                    if(String.Compare(arr1[j1], arr2[j2]) < 0)
                    {
                        result[i] = arr1[j1++];
                    }
                    else
